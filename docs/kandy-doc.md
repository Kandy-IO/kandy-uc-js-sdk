...
initialization of the SDK using the [config.call.sdpHandlers][86] configuration
parameter. They can also be set after the SDK's creation by using the
[call.setSdpHandlers][87] function.
...
The SDK will emit a [users:change][89]
...
Compared to [user.fetch][91] API, this API retrieves additional user related information.
...
See the [user.fetch][91] and [user.search][92] APIs for details about
...
Returns **[user.User][93]** The User object for the specified user.
...
Returns **[Array][12]&lt;[user.User][93]>** An array of all the User objects.
...
The SDK will emit a [directory:change][94]
   ...
   [user.get][90] and [user.getAll][95] APIs.
