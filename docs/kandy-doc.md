...
Starts an outgoing call to a [SIP URI][9] or a
   [TEL URI][10].
...
The [call.getById][11] API can be used to retrieve
   the current information about the call.

The SDK will emit a [call:start][12] event
   locally when the operation completes. When the remote participant
   receives the call, a [call:receive][13]
...
The SDK will emit a [call:stateChange][16]
   ...
   [call.getById][17] API can be used to retrieve the
   ...
   See the [call:newTrack][18] event for
...
Some call operations cannot be performed while the call is on hold. The
   call can be taken off hold with the [call.unhold][19] API.
...
The SDK will emit a [call:newTrack][18] event
...
The Call's state is a property of the [CallObject][23], which can be
   retrieved using the [call.getById][17] or
   [call.getAll][24] APIs.
...
-   `trackIds` **[Array][8]&lt;[string][2]>** List of Track IDs to be rendered.
...
-   `trackIds` **[Array][8]&lt;[string][2]>** List of Track IDs to stop being rendered.
-   `cssSelector` **[string][2]** A CSS selector string that uniquely
       identifies an element. Ensure that special characters are properly
       escaped.
...
-   `trackIds` **[Array][8]&lt;[string][2]>** List of Track IDs.
...
Returns **[Array][8]&lt;[User][48]>** An array of all the User objects.
...
-   `camera` **[Array][8]&lt;[DeviceInfo][56]>** A list of camera device information.
-   `microphone` **[Array][8]&lt;[DeviceInfo][56]>** A list of microphone device information.
-   `speaker` **[Array][8]&lt;[DeviceInfo][56]>** A list of speaker device information.
...
[11]: Calls.getCallById

[12]: #callseventcallstart

[13]: #callseventcallreceive
