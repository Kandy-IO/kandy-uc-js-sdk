...
initialization of the SDK using the [config.call.sdpHandlers][119] configuration
...
[sip.subscribe API][121] for more information about solicited events.
...
from a subscription using the [sip.update][122] API at any time.

The SDK will emit a [sip:subscriptionChange][123]
event when the operations completes. The [sip.getDetails][124] API can be used
to retrieve the current information about a subscription.

The SDK will emit a [sip:eventsChange][125] event when
...
*   `customParameters` **[Array][13]<[call.CustomParameter][33]>?** Custom SIP header parameters for the SIP backend.
...
Subscription details will no longer be available using the [sip.getDetails][124]
...
This event can be emitted when a new SIP subscription is created ([sip.subscribe][121]
API), an existing subscription is updated ([sip.update][122] API), or has been
deleted ([sip.unsubscribe][126] API). The `change` parameter on the event indicates
...
The [sip.getDetails][124] API can be used to retrieve the current information about
a subscription.
...
*   `userId` **[user.UserID][29]** The User ID of the user.
...
The SDK will emit a [users:change][127]
...
Compared to [user.fetch][129] API, this API retrieves additional user related information.
...
See the [user.fetch][129] and [user.search][130] APIs for details about
...
Returns **[user.User][131]** The User object for the specified user.
...
Returns **[Array][13]<[user.User][131]>** An array of all the User objects.
...
The SDK will emit a [directory:change][132]
...
[user.get][128] and [user.getAll][133] APIs.

#### Parameters

*   `filters` **[Object][7]** The filter options for the search.

    *   `filters.userId` **[user.UserID][29]?** Matches the User ID of the user.
...
*   `params` **[Object][7]** 
    ...
    *   `params.results` **[Array][13]<[user.User][131]>** The Users' information returned by the
        operation.
...
A [voicemail:change][134] event is
...
[22]: https://developer.mozilla.org/en-US/docs/Web/API/fetch
...
[123]: #sipeventsipsubscriptionchange

[124]: #sipgetdetails

[125]: #sipeventsipeventschange

[126]: #sipunsubscribe

[127]: #usereventuserschange
...
[132]: #usereventdirectorychange

[133]: #usergetall

[134]: #voicemaileventvoicemailchange
