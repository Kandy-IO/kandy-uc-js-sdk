...
These parameters can be specified with the [call.make][34] and [call.answer][35] APIs.
They can also be set after a Call is established using the [call.setCustomParameters][36] API, and sent using the [call.sendCustomParameters][37] API.
...
A Call's custom parameters are stored on the Call's [CallObject][39],
 ...
 part of the CallObject and are only provided via the [call:customParameters][38] event.
...
Starts an outgoing call to a [SIP_URI][40] or a
   [TEL_URI][41].
...
The SDK will emit a [call:start][43] event
   locally when the operation completes. When the remote participant
   receives the call, a [call:receive][44]
...
-   `destination` **([call.SIP_URI][45] \| [call.TEL_URI][46])** The desired destination.
-   `media` **[Object][7]** The media options the call should be initialized with.
    ...
    -   `media.audioOptions` **[Object][7]?** Options for configuring the call's audio.
        -   `media.audioOptions.deviceId` **[call.MediaConstraint][47]?** ID of the microphone to receive audio from.
    ...
    -   `media.videoOptions` **[Object][7]?** Options for configuring the call's video.
        -   `media.videoOptions.deviceId` **[call.MediaConstraint][47]?** ID of the camera to receive video from.
        -   `media.videoOptions.height` **[call.MediaConstraint][47]?** The height of the video.
        -   `media.videoOptions.width` **[call.MediaConstraint][47]?** The width of the video.
        -   `media.videoOptions.frameRate` **[call.MediaConstraint][47]?** The frame rate of the video.
    ...
    -   `media.screenOptions` **[Object][7]?** Options for configuring the call's screenShare.
        -   `media.screenOptions.height` **[call.MediaConstraint][47]?** The height of the screenShare.
        -   `media.screenOptions.width` **[call.MediaConstraint][47]?** The width of the screenShare.
        -   `media.screenOptions.frameRate` **[call.MediaConstraint][47]?** The frame rate of the screenShare.
-   `options` **[Object][7]?** 
...
The SDK will emit a [call:stateChange][49]
   ...
   [call:newTrack][50] event for more
...
The SDK will emit a [call:newTrack][50] event
...
-   `videoOptions` **[Object][7]?** Options for configuring the call's video.
    -   `videoOptions.deviceId` **[call.MediaConstraint][47]?** ID of the camera to receive video from.
    -   `videoOptions.height` **[call.MediaConstraint][47]?** The height of the video.
    -   `videoOptions.width` **[call.MediaConstraint][47]?** The width of the video.
    -   `videoOptions.frameRate` **[call.MediaConstraint][47]?** The frame rate of the video.
-   `options` **[Object][7]?** 
    -   `options.bandwidth` **[call.BandwidthControls][27]?** Options for configuring media's bandwidth.
    -   `options.dscpControls` **[call.DSCPControls][48]?** Options for configuring DSCP markings on the media traffic.
...
The data retrieved is a [RTCStatsReport][57]
...
The SDK will emit a
   [call:statsReceived][59] event, after
...
-   `callId` **[string][8]** ID of the call being acted on.
-   `destination` **([call.SIP_URI][45] \| [call.TEL_URI][46])** The destination to forward the call to.
...
The progression of the operation will be tracked via the
   [call:operation][42] event. The remote
...
-   `callId` **[string][8]** ID of the call being acted on.
-   `destination` **([call.SIP_URI][45] \| [call.TEL_URI][46])** The destination to transfer the call to.
...
The progress of the operation will be tracked via the
   [call:operation][42] event. Both remote
...
In addition, the SDK emits a [call:availableCodecs][61] event
 upon retrieving that list of codecs.

This API is a wrapper for the static method [RTCRtpSender.getCapabilities()][62].
...
Set [SDP Handler Functions][63] that will be run as part of a pipeline for all future calls.
...
[CallHistory.get][67] can then be used to get
...
The data is provided in a format that can be used directly with the
   [call.history.setCache][68] API. This allows an
...
The data can be retreived from the [   call.history.getCache][69] API. This allows an
   application to persist the information across SDK instances when the
   backend environment does not support the CallHistory feature.
...
Returns **[conversation.Conversation][70]** A Conversation object matching the passed destination, otherwise undefined is returned.
...
A message object can be obtained through the [Conversation.createMessage][71] API on an existing conversation.
...
the message can then be sent using the [Message.send][74] function.
...
obtained through the [Conversation.getMessages][75] or [Conversation.getMessage][76] API on an existing conversation.
...
-   `type` **[string][8]** The type of message that was sent. See [conversation.chatTypes][77] for valid types.
...
The SDK has an internal logging system for providing information about its
   ...
   [config.logs][78]) to customize how the logs are
...
For device information, the [media.getDevices][80] API will retrieve
   ...
   recommend using the [media.initializeDevices][83] API in this
...
The SDK will emit a [devices:change][81]
   event when the operation is successful or a
   [devices:error][84] event if an error is
   encountered.
...
Presence information is persisted by the server. When the SDK is initialized,
   ...
   available either by using [presence.fetch][89] or by subscribing for
   updates about other Users, using [presence.subscribe][90].

Available presence information can be retrieved using [presence.get][91] or
   [presence.getAll][92].
...
An instance of this type can be obtained by invoking the [presence.get][91] function.
...
The status is set to [open][93] as soon as a user subscribes for the presence service.
...
-   `status` **[string][8]** The current status the user has set for themselves. For supported values see [presence.statuses][93].
-   `activity` **[string][8]** The current activity of the user.
         For supported values see [presence.activities][94].
...
See [presence.statuses][93] and [presence.activities][94] for valid
   values.

The SDK will emit a
   [presence:selfChange][95] event
   when the operation completes. The updated presence information is
   available and can be retrieved with [presence.getSelf][96].

Other users subscribed for this user's presence will receive a
   [presence:change][97] event.
...
This information is set using the [presence.update][98] API.
...
Available presence information an be retrieved using the
   [presence.get][91] or [presence.getAll][92] APIs.
...
A set of [SdpHandlerFunction][63]s for manipulating SDP information.
...
removal handler in combination with the [call.getAvailableCodecs][101] function in order to build logic to determine the best codecs to use
...
A SIP event may either be solicited or unsolicited. Solicited events, such as the "presence"
   ...
   [sip.subscribe API][102] for more information about solicited events.
...
Only one SIP subscription per event type can exist at a time. A subscription can
   ...
   from a subscription using the [sip.update][103] API at any time.

The SDK will emit a [sip:subscriptionChange][104]
   event when the operations completes. The [sip.getDetails][105] API can be used
   to retrieve the current information about a subscription.

The SDK will emit a [sip:eventsChange][106] event when
...
Subscription details will no longer be available using the [sip.getDetails][105]
...
Compared to [user.fetch][109] API, this API retrieves additional user related information.
...
Returns **[Array][13]&lt;[user.User][111]>** An array of all the User objects.
...
A [voicemail:change][114] event is
...
[43]: #calleventcallstart

[44]: #calleventcallreceive

[45]: #callsip_uri

[46]: #calltel_uri

[47]: #callmediaconstraint

[48]: #calldscpcontrols

[49]: #calleventcallstatechange

[50]: #calleventcallnewtrack
...
[57]: https://developer.mozilla.org/en-US/docs/Web/API/RTCStatsReport

[58]: https://developer.mozilla.org/en-US/docs/Web/API/RTCStats

[59]: #calleventcallstatsreceived

[60]: #calleventcalltrackreplaced

[61]: #calleventcallavailablecodecs

[62]: https://w3c.github.io/webrtc-pc/#dom-rtcrtpsender-getcapabilities

[63]: #callsdphandlerfunction
...
[68]: callHistory.setCache

[69]: callHistory.getCache

[70]: #conversationconversation

[71]: #conversationconversationcreatemessage

[72]: #conversationmessage

[73]: #conversationmessageaddpart

[74]: #conversationmessagesend

[75]: #conversationconversationgetmessages

[76]: #conversationconversationgetmessage

[77]: conversation.chatTypes
...
[83]: #mediainitializedevices

[84]: #mediaeventdeviceserror
...
[94]: #presenceactivities

[95]: #presenceeventpresenceselfchange

[96]: #presencegetself

[97]: #presenceeventpresencechange
...
[101]: #callgetavailablecodecs

[102]: #sipsubscribe

[103]: #sipupdate

[104]: #sipeventsipsubscriptionchange

[105]: #sipgetdetails

[106]: #sipeventsipeventschange
...
[114]: #voicemaileventvoicemailchange
